...
class Distribution(ABC, Generic[A]):
    '''To model a probability distribution class'''
    ...
    def expectation(
        ...
        for the distribution and f is an arbitrary function from X to float'''
        pass
    ...
    def map(
        ...
        '''Apply a function that returns a distribution to the outcomes of this 
        distribution. This lets us express *dependent random variables*
        '''
        def sample():
            a = self.sample()
            b_dist = f(a)
...
class Gaussian(SampledDistribution[int]):
    '''A gaussian given μ and σ '''
    μ: float
    σ: float
...
class Gamma(SampledDistribution[int]):
    '''A Gamma Distribution with the given α and β.'''
    α: float
    β: float
    
    def __init__(self, α: float, β: float, expectation_smaples: int = 10000):
        self.α = α
        self.β = β
        super().__init__(
...
class Categorical(FiniteDistribution[A]):
    '''Select from a finite set out outcomes with the specified probabilities'''
