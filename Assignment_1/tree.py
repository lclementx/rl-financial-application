...
for l in lucks:
    a = ta.perform_action()
    actions.append(a * ta.current.W)
    ta.transition(l)
print('Reward from optimal agent base on tree:', compute_utility(ta.current.W))
print('Actions taken by optimal agent base on tree:', actions)