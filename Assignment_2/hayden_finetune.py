...
def generate_simulation(policy):
    ...
    for _ in range(9):
        ...
        cash = cash * risk_free_factor
        up_price = current_price * up_factor
        down_price = current_price * down_factor
        previous_prices.append(current_price)
        up_prices.append(current_price * up_factor)
        down_prices.append(current_price * down_factor)
        preaction_cashes.append(cash)
        preaction_holdings.append(holding)

        # Perform re-balancing
        hedge_ratio = policy(up_price, down_price, exercise_price, cash, risk_free_factor)
        cash -= (hedge_ratio - holding) * current_price
        holding = hedge_ratio

        # Record position after re-balancing
        preaction_cashes.append(cash)
        preaction_holdings.append(holding)
    ...
    return up_prices, down_prices, previous_prices, next_prices, preaction_holdings, postaction_holdings
